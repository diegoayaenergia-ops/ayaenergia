...
function FullscreenToggle({ active, compact, onToggle }: { active: boolean; compact?: boolean; onToggle: () => void }) {
  return (
    <Btn tone="secondary" onClick={onToggle} className={cx(compact ? "h-9 px-3 text-xs" : "")} title={active ? "Sair" : "Tela cheia"}>
    ...
    <div className="fixed inset-0 z-[999] flex flex-col" style={{ background: T.bg, color: T.text }}>
      ...
      >
        <div className="min-w-0">
          ...
          {hint ? <div className="mt-1 text-xs" style={{ color: T.text3 }}>{hint}</div> : null}
        </div>
      ...
      {filtersOpen ? <div className="shrink-0 border-b" style={{ borderColor: T.border, background: T.card }}>{filters}</div> : null}
...
function StationPicker({
  ...
  return (
    <div ref={wrapRef} className="relative min-w-0">
      <label className={UI.label} style={{ color: T.text2 }}>Usina</label>
      ...
      {open && (
        <div className="absolute z-[300] mt-2 w-full border rounded-lg shadow-sm overflow-hidden" style={{ borderColor: T.border, background: T.card }}>
          ...
          <div className="max-h-80 overflow-auto">
            {list.length === 0 ? (
              <div className="px-3 py-3 text-sm" style={{ color: T.text2 }}>Nenhuma usina encontrada.</div>
            ) : (
              list.map((s, idx) => {
...
function MiniChart({
  ...
  const barW = Math.max(6, Math.min(18, (xBand * 0.70) / barCount));
  ...
  return (
    <div ref={ref} className="min-w-0">
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          <div className={UI.cardTitle} style={{ color: T.text }}>{title}</div>
          {subtitle ? <div className="mt-1 text-[11px]" style={{ color: T.text3 }}>{subtitle}</div> : null}
        </div>
      </div>
  ...
  return (
    <div className="border rounded-lg p-4 min-w-0" style={{ borderColor: T.border, background: T.card }}>
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className={UI.label} style={{ color: T.text2 }}>{title}</div>
          <div className="mt-1 text-2xl font-extrabold truncate" style={{ color: T.text }}>{value}</div>
          <div className="mt-1 text-[11px] truncate" style={{ color: T.text3 }}>{sub || " "}</div>
        </div>
        <div className="shrink-0" style={{ color: T.text3 }}>{icon}</div>
      </div>
...
export function TecsciPage() {
  ...
  const [filtersOpen, setFiltersOpen] = useState(false); // ✅ por padrão FECHADO (menos poluição)
  ...
  const seriesAvail = useMemo(() => filtered.map((b) => ({ periodo: b.label, disponibilidade: safeNum(b.dto.availability_percentage), meta: 97 })), [filtered]);
    ...
    <div className="p-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-3 items-end">
        ...
        <div className="lg:col-span-2">
          ...
          <input className={UI.input} style={{ borderColor: T.border }} type="date" value={start} onChange={(e) => { setStart(e.target.value); setPeriodPreset("custom"); }} />
        </div>

        <div className="lg:col-span-2">
          <label className={UI.label} style={{ color: T.text2 }}>Fim</label>
          <input className={UI.input} style={{ borderColor: T.border }} type="date" value={end} onChange={(e) => { setEnd(e.target.value); setPeriodPreset("custom"); }} />
        </div>

        <div className="lg:col-span-2">
          <label className={UI.label} style={{ color: T.text2 }}>Group</label>
        ...
        <div className="lg:col-span-2">
          <label className={UI.label} style={{ color: T.text2 }}>Energia</label>
          <select className={UI.select} style={{ borderColor: T.border }} value={energyUnit} onChange={(e) => setEnergyUnit(e.target.value as any)}>
        ...
        <div className="lg:col-span-8">
          <label className={UI.label} style={{ color: T.text2 }}>Buscar período</label>
          <div className="relative">
            <input className={cx(UI.input, "pr-9")} style={{ borderColor: T.border }} value={searchText} onChange={(e) => setSearchText(e.target.value)} placeholder="Ex: 2026-01, 02/2026..." />
            <div className="absolute right-2.5 top-1/2 -translate-y-1/2" style={{ color: T.text3 }}>
              <Search className="w-4 h-4" />
            </div>
          </div>
  ...
  // ✅ ChartsBody (ÚNICO, sem duplicação)
  const ChartsBody = (
    <div className="p-4 grid gap-4">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 items-start">
        ...
        {/* deixa irradiação como leitura secundária, mas ainda disponível */}
  ...
  const TableBody = (
    <div className="p-4">
      ...
      <div className="mt-3 border rounded-lg overflow-hidden" style={{ borderColor: T.border }}>
        ...
        {tableRows.length === 0 ? (
          <div className="px-3 py-6 text-sm" style={{ color: T.text2 }}>Sem dados.</div>
        ) : (
          tableRows.map((r, idx) => (
            <div
              key={`${r.periodo}-${idx}`}
            ...
            >
              ...
              <div className={UI.mono} style={{ textAlign: "right" }}>{r.generated_kwh != null ? brInt(r.generated_kwh) : "—"}</div>
              <div className={UI.mono} style={{ textAlign: "right" }}>{r.expected_kwh != null ? brInt(r.expected_kwh) : "—"}</div>
              <div className={UI.mono} style={{ textAlign: "right" }}>{r.p90_kwh != null ? brInt(r.p90_kwh) : "—"}</div>
              <div className={UI.mono} style={{ textAlign: "right" }}>{r.poa_kwh_m2 != null ? brNum(r.poa_kwh_m2, 2) : "—"}</div>
              <div className={UI.mono} style={{ textAlign: "right" }}>{r.projected_pr != null ? brNum(r.projected_pr, 2) : "—"}</div>
              <div className={UI.mono} style={{ textAlign: "right" }}>{r.dc_power_kw != null ? brNum(r.dc_power_kw, 2) : "—"}</div>
              <div className={UI.mono} style={{ textAlign: "right", fontWeight: 900 }}>{r.estimated_aya_kwh != null ? brInt(r.estimated_aya_kwh) : "—"}</div>
              <div className={UI.mono} style={{ textAlign: "right", fontWeight: 900, color: r.loss_aya_kwh != null && r.loss_aya_kwh > 0 ? T.errTx : T.text }}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-3 items-stretch">
        <div className="lg:col-span-4">
          <KpiCard
            title="Usina"
            value={kpi.psLabel}
            sub={`Período: ${brDate(start)} → ${brDate(end)}`}
        ...
        <div className="lg:col-span-4">
          <KpiCard
            title={`P90 (${energyUnit})`}
            value={kpi.p90Kwh != null ? brNum(kwhToUnit(kpi.p90Kwh, energyUnit), 2) : "—"}
            sub="projected_energy_kwh"
        ...
        <div className="lg:col-span-4">
          <KpiCard
            title={`Perdas AYA (${energyUnit})`}
            value={kpi.lossAyaKwh != null ? brNum(kwhToUnit(kpi.lossAyaKwh, energyUnit), 2) : "—"}
            sub="max(0, Estimado AYA − Gerado)"
            icon={<Activity className="w-4 h-4" />}
          />
        </div>
        ...
        <div className="lg:col-span-4">
          <KpiCard
            title="Notas"
            value={data?.group || "—"}
            sub={`group (resposta): ${data?.group || "—"} • group (req): ${group}`}
            icon={<Gauge className="w-4 h-4" />}
          />
        </div>
      </div>
  ...
  const TableSection = (
    <div className={cx(UI.section, "rounded-lg")} style={{ borderColor: T.border, background: T.card }}>
      <SectionHeader
        title="Tabela (Analista)"
        hint="Export CSV"
        right={<FullscreenToggle active={false} compact={isMobile} onToggle={() => toggleFull("table")} />}
      />
      {TableBody}
    </div>
  );
  ...
  return (
    <section className={UI.page} style={{ background: T.bg, color: T.text }}>
      <div className={UI.container}>
        ...
        {/* QUICK FILTER BAR (menos poluição) */}
