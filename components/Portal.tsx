...
/* =========================================================
   TOP DROPDOWN (hover + click sticky)
========================================================= */
    ...
    return (
        ...
        >
            <button
                ...
                className={cx(
                    "h-9 px-3 rounded-xl text-[13px] font-semibold text-white/92 hover:bg-white/10 transition",
...
function Drawer({
    ...
    const suggestionList = q ? results : recentItems;
                    ...
                    <div className="p-4 border-b" style={{ borderColor: UI.border }}>
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="w-full h-11 pl-10 pr-3 rounded-2xl border border-black/10 bg-white text-sm outline-none focus-visible:ring-2 focus-visible:ring-[rgba(47,178,106,0.35)]"
                            />
                        </div>

                        <div className="mt-3 rounded-2xl border overflow-hidden" style={{ borderColor: UI.border }}>
                            <div className="px-3 py-2 text-[12px] font-semibold text-black/70 bg-black/[0.02]">{q ? "Resultados" : "Recentes"}</div>

                            {suggestionList.length ? (
                                <div className="max-h-[240px] overflow-y-auto">
                                    {suggestionList.map((it) => {
                                        const Icon = it.icon || SolarPanel;
                                        return (
                                            <button
                                                key={it.id}
                                                type="button"
                                                onClick={() => onGo(it.id)}
                                                className="w-full px-3 py-3 flex items-center gap-3 text-left hover:bg-black/[0.03]"
                                            >
                                                <span className="w-9 h-9 rounded-xl bg-black/[0.03] grid place-items-center text-black/60">
                                                    {q ? <Icon className="w-4 h-4" /> : <History className="w-4 h-4" />}
                                                </span>
                                                <div className="min-w-0 flex-1">
                                                    <div className="text-sm font-semibold text-black/85 truncate">{it.title}</div>
                                                    <div className="text-[11px] text-black/45 truncate">{it.group}</div>
    ...
    return (
        <div className="h-screen w-full overflow-hidden bg-white text-black flex flex-col">
            <Drawer
                ...
                recentItems={recentItems.length ? recentItems : allAllowedItems.slice(0, 6)}
                onGo={go}
                onSupport={() => {
                    setDrawerOpen(false);
                    setShowSupport(true);
            ...
            <header className="sticky top-0 z-30">
                <div className="h-16 px-3 md:px-4 flex items-center gap-2" style={{ background: UI.top }}>
                    ...
                    >
                        <Image src="/logo-aya.png" alt="AYA" width={34} height={34} />
                    ...
                    {/* SEARCH (desktop) */}
                    <div ref={searchWrapRef} className="relative hidden md:block flex-1 max-w-[900px] mx-3">
